...
## Branding (Icons)
If a repo includes branding files, YidStore will install them automatically so Home Assistant can show local branding.

Supported paths:
- `custom_components/<domain>/brand/icon.png` (preferred)
- `custom_components/<domain>/brand/logo.png`
- `custom_components/<domain>/icon.png` (legacy fallback)
